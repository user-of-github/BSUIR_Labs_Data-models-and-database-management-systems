...
BEGIN
    DBMS_OUTPUT.PUT_LINE('[compare_schemas()] Started executing');
    ...
    FOR table_from_1_schema IN tables_from_1_schema LOOP
        ...
        FOR table_from_2_schema IN tables_from_2_schema LOOP
            ...
            IF table_name_1 = table_name_2_temp THEN -- Table exists in both schemas ===> compare table structure
                ...
                IF are_tables_equal = false THEN
                    DBMS_OUTPUT.PUT_LINE('[compare_schemas()] Table ' || table_name_1 || ' exists in both schemas ( ' || schema_1_name || ' and ' || schema_2_name || ') , but has DIFFERENT structures');
                END IF;
        ...
        IF found_table_flag = false THEN
            DBMS_OUTPUT.PUT_LINE('[compare_schemas()] Table ' || table_name_1 || ' exists in ' || schema_1_name || ' , but not in ' || schema_2_name);
        END IF; 
